...
let TodoList = [];
  ...
  const li = $(<li><input id='lishka' type='checkbox'> <span> <img src='icon.png' class='item_${j}' ></img></span>"+DeskTaskInput.val()+"</li>")
...
if ($('#lishka').is(':checked')) {
  $('body input:checkbox').prop('checked', false);
  $('lishka').remove('TodoList[i]');
}
else { $('body input:checkbox').prop('checked', false);
}
};