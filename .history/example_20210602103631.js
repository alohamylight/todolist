...
$('#btn-off').click(function(){
  ...
  $(`#item_${Date.now()}`).on("click","button",taskRemove)  const taskRemove = function(){
    $(`#item_${Date.now()}`).on("click","button",taskRemove)
    const id = $(this).closest(`#item_${Date.now()}`).attr("id")
