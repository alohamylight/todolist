...
$("html").on('click',".123",function(){
  for (j = 0; j< TodoList.length; ++j) {
    console.log(TodoList.length - 1.checked);
    if(TodoList[TodoList.length - 1].checked == false)
    {
      TodoList[TodoList.length - 1].checked = true;
     }
    else {
      TodoList[TodoList.length - 1].checked = false;}
  }
});
let massive= []
function MyFunc(){
