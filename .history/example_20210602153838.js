...
let delete__all =function(){
  TodoList.forEach(element => {
    if ($('body input:checkbox').is(':checked'))  мы проверяем все элементы массива? если какой то чекедб тогда этот элемент записывается в переменную и этот элемент массива удаляется
    {
     pam = $('body input:checkbox').is(':checked');
     console.log(pam);
    }
    else {}
  })};
